			...
			for(String each:paramsTemp) {
				...
				if(kv.length==2) {
					if(params.containsKey(kv[0])) {
						try {
							params.get(kv[0]).add(URLDecoder.decode(kv[1],"utf-8"));
							//System.out.println("转移后"+URLDecoder.decode(kv[1],"utf-8"));
						} catch (UnsupportedEncodingException e) {
							// TODO Auto-generated catch block
							e.printStackTrace();
						}
						...
						try {
							t.add(URLDecoder.decode(kv[1],"utf-8"));
						} catch (UnsupportedEncodingException e) {
							// TODO Auto-generated catch block
							e.printStackTrace();
						}
